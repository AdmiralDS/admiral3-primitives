...
- экспорт базовых UI-компонентов через единый публичный entrypoint;
...
Публичный `exports` в `package.json` намеренно оставляет только root entrypoint пакета. Deep imports из внутренних папок не являются частью публичного API.
...
- `scripts/generate-react-component.mjs` - тонкая обвязка над `generate-react-cli`: принимает имя компонента аргументом или спрашивает его в интерактивном терминале, запускает генерацию component files, переносит story/template в `stories`, создает playground scenario и e2e smoke-тест, обновляет root export/playground aggregator.
- `scripts/validate-components.mjs` - Node-скрипт структурной проверки компонентов и публичного API. Проверяет обязательные component files, Storybook entrypoint, root-only package exports, root export, локальный экспорт компонента и публичных props/types, запрет внутренних constants/styles/styled props в API, связь playground-сценария с e2e-спекой и импорты публичных props/types в story/template файлах.
- `scripts/validate-package.mjs` - Node-скрипт проверки npm tarball через `npm pack --dry-run --json`. Контролирует, что в publish-состав не попадают исходники, Storybook/Playwright артефакты, templates и локальные служебные файлы, а `dist/index.js` остается в lightweight size limit.
...
- `vite.config.ts` - Vite library build config. Собирает ES entrypoint `index`, задает aliases и external dependencies: tokens package, React и `styled-components`.
...
- `check:full` - полный обязательный локальный прогон: format check, lint, component structure validation, typecheck, unit и e2e.
